...
#include <RMSNorm/RMSNorm.cuh>
#include <vector>
#include <algorithm>


void bench_rmsnorm(nvbench::state &state) {
  ...
  thrust::device_vector<half> weight(hidden_dim);
  ...
  state.add_global_memory_reads<half>(
    hidden_states.size() + weight.size() + reorder_index.size(),
    "Read"
  );
  ...
  state.exec(nvbench::exec_tag::timer, [&](nvbench::launch &launch, auto &timer) {
    timer.start();
    run_rmsnorm_fp16_i4<group_size, hidden_dim>(
