...
struct vec_t {
  ...
  template <typename T>
  FLASHINFER_INLINE void cast_load(const T *ptr, float scale, float zero);
};

// We just use __precision__s4 as identifier. Never used directly.
namespace quant{
...
FLASHINFER_INLINE void cast_from_impl(const vec_t<src_float_t, vec_size> &src,
  ...
  static_assert(std::is_same<src_float_t, quant::__precision__s4>::value == false);
#pragma unroll
...
FLASHINFER_INLINE void cast_load_impl(const src_float_t *src_ptr,
                                      vec_t<tgt_float_t, vec_size> &dst,
                                      float scale = 0.0f, float zero = 0.0f) {
  if constexpr (std::is_same<src_float_t, tgt_float_t>::value) {
    dst.load(src_ptr);
  }else if constexpr(std::is_same<src_float_t, quant::__precision__s4>::value){
    vec_t<src_float_t, vec_size> tmp;
    tmp.load(src_ptr);
    quant::dequantize_impl(tmp, dst, scale, zero);
  }else{
    vec_t<src_float_t, vec_size> tmp;
    tmp.load(src_ptr);
    dst.cast_from(tmp);
...
/******************* vec_t<__precision__s4> *******************/
// __precision__s4 x 8
// 32 bits. Used for decrease the computation load.
template <>
struct vec_t<quant::__precision__s4, 8> {
  uint32_t data[1];
  FLASHINFER_INLINE void load(const quant::__precision__s4 *ptr) {
    data[0] = ((uint32_t *)ptr)[0];
  }

  // Get the i-th element of packed uint32_t register
  FLASHINFER_INLINE uint32_t &at(size_t i){
    return ((uint32_t *)data)[i];
  }

  FLASHINFER_INLINE const uint32_t &at(size_t i) const{
    return ((uint32_t *)data)[i];
  }
};

// __precision__s4 x 16
// 64 bits. Used for decrease the computation load.
template <>
struct vec_t<quant::__precision__s4, 16> {
  uint32_t data[2];
  FLASHINFER_INLINE void load(const quant::__precision__s4 *ptr) {
    data[0] = ((uint32_t *)ptr)[0];
    data[1] = ((uint32_t *)ptr)[1];
  }
...
// __precision__s4 x 32 or more
template <size_t vec_size>
struct vec_t<quant::__precision__s4, vec_size> {
  uint4 data[vec_size / 32];
...
#pragma unroll
  ...
  FLASHINFER_INLINE static void memcpy(quant::__precision__s4 *dst, const quant::__precision__s4 *src){
#pragma unroll
    for (size_t i = 0; i < vec_size / 32; ++i) {
...
#pragma unroll
  ...
  template <typename T>
  FLASHINFER_INLINE void cast_load(const T *ptr, float scale, float zero){
    cast_load_impl(ptr, *this, scale, zero);
  }
};
