...
void batch_decode(torch::Tensor o, torch::Tensor q, torch::Tensor kv_data,
  ...
  FlashInferBatchDecodeKernel<128, __nv_fp8_e4m3>(                           
      static_cast<__nv_fp8_e4m3*>(o.data_ptr()),                             
      static_cast<__nv_fp8_e4m3*>(q.data_ptr()),                             
      static_cast<__nv_fp8_e4m3*>(kv_data.data_ptr()),                       
...
void init_kv(torch::Tensor kv_data, torch::Tensor kv_indptr,
  ...
  FlashInferInitKvKernel<128, __nv_fp8_e4m3>(                                       
      static_cast<__nv_fp8_e4m3*>(kv_data.data_ptr()),                              
      kv_indptr.data_ptr<int32_t>(), kv_indicies.data_ptr<int32_t>(),        
      last_page_offset.data_ptr<int32_t>(),                                  
      static_cast<__nv_fp8_e4m3*>(k.data_ptr()),                                    
      static_cast<__nv_fp8_e4m3*>(v.data_ptr()), seqlen_indptr.data_ptr<int32_t>(), 
...
void append_kv(torch::Tensor kv_data, torch::Tensor kv_indptr,
  ...
  FlashInferAppendKvKernel<128, __nv_fp8_e4m3>(                                    
      static_cast<__nv_fp8_e4m3*>(kv_data.data_ptr()),                            
      kv_indptr.data_ptr<int32_t>(), kv_indicies.data_ptr<int32_t>(),       
      last_page_offset.data_ptr<int32_t>(),                                 
      static_cast<__nv_fp8_e4m3*>(k.data_ptr()),                                   
      static_cast<__nv_fp8_e4m3*>(v.data_ptr()), num_layers, layer_idx, num_heads,
...
torch::Tensor dispatch_gemm_forward(torch::Tensor _in_feats,
                                    torch::Tensor _kernel,
                                    torch::Tensor _scaling_factors,
                                    torch::Tensor _zeros) {
  return gemm_forward_cuda(_in_feats, _kernel, _scaling_factors, _zeros, 128,
                           8);
}

torch::Tensor dispatch_gemv_forward(torch::Tensor _in_feats,
                                    torch::Tensor _kernel,
                                    torch::Tensor _scaling_factors,
                                    torch::Tensor _zeros) {
  return gemv_forward_cuda(_in_feats, _kernel, _scaling_factors, _zeros, 128);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("dispatch_gemv_forward", &dispatch_gemv_forward, "");
  m.def("dispatch_gemm_forward", &dispatch_gemm_forward, "");
}
