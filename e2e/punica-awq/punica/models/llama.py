...
class LinearInt4(nn.Module):
  ...
  def __init__(self, in_features, out_features, bias=False):
    ...
    self.weight = nn.Parameter(
        torch.empty(
            out_features, make_divisible(in_features, 8), dtype=torch.int32),
        requires_grad=False)
    self.scale = nn.Parameter(
        torch.empty(
            out_features, make_divisible(in_features, group_size), dtype=torch.float16),
        requires_grad=False)
    self.zeros = nn.Parameter(
        torch.empty(
            out_features, make_divisible(make_divisible(in_features, group_size), 8), dtype=torch.int32),
        requires_grad=False
      )
  ...
  def forward(self, input):
    assert(input.dim() == 2)
    # Follow:https://github.com/mit-han-lab/llm-awq/blob/main/awq/quantize/qmodule.py
    if(input.shape[0] <= 8):
      result = gemv_forward(input, self.weight, self.scale, self.zeros)
    else:
      result = gemm_forward(input, self.weight, self.scale, self.zeros)
...
class LlamaMLP(nn.Module):
  def __init__(self, config):
    ...
    self.gate_proj = LinearInt4(self.hidden_size, self.intermediate_size, bias=False)
    self.up_proj = LinearInt4(self.hidden_size, self.intermediate_size, bias=False)
    self.down_proj = LinearInt4(self.intermediate_size, self.hidden_size, bias=False)
