...
def textgen_punica(model_cfg: ModelConfig, textgen_cfg: TextGenConfig,
  ...
  from punica.utils import BatchLenInfo, KvPoolInt4, KvCacheInt4, BatchedKvCacheInt4
  ...
  while len(done) != len(rs):
    ...
    while (len(workset) + len(newreqs) < textgen_cfg.batch_size and
      ...
      kvcache = KvCacheInt4(kvpool, prompt_len)
    ...
    prefill_kv = BatchedKvCacheInt4(prefill_kv) if newreqs else None
    decode_kv = BatchedKvCacheInt4([req.kvcache for req in workset
