...
class KvPoolInt4:
  ...
  def __init__(
    ...
    self._buf = torch.empty(
        (capacity, num_layers, 2, num_heads, block_len, head_dim // 2),
        dtype=torch.uint8,
        device=device)
    self._param = torch.empty(
        (capacity, num_layers, 2, num_heads, block_len, 2),
        dtype=torch.float16,
        device=device)
...
class KvCacheInt4:
  ...
  def __init__(self, pool: KvPoolInt4, init_len: int):
    if init_len < 0:
...
class BatchedKvCacheInt4:
  """Key-value cache for a batch of sequences."""

  def __init__(self, kv: Sequence[KvCacheInt4]):
    assert len(kv) > 0
    pool = kv[0].pool
